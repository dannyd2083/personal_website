...
const FilterGallery = ({photos, query, onPhotoClick}) => {
    if(!photos) return;
    return(
        <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                {photos.map(photo => (
                    <div key={photo.id} className="relative aspect-[4/3] rounded-lg overflow-hidden cursor-zoom-in" onClick={() => onPhotoClick?.(photo)}>
                        ...
                        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent px-2 py-2">
                            ...
                            <span className="text-white/60 text-xs ml-1">· {photo.visit_name ?? photo.country}</span>
                        </div>
                    </div>
                ))}
            </div>
